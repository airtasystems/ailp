...
const ASSESSMENT_MODE_VALUES: readonly AilpAssessmentMode[] = [
  "response_safety",
...
function coerceAssessmentMode(raw: string | undefined): AilpAssessmentMode | undefined {
  if (raw == null) return undefined;
  const t = raw.trim().toLowerCase().replace(/-/g, "_");
  if (t === "") return undefined;
  if (t === "response" || t === "safety") return "response_safety";
  if (t === "security" || t === "request_security" || t === "response_security") {
    return "response_safety_with_request_security";
  }
  return (ASSESSMENT_MODE_VALUES as readonly string[]).includes(t)
    ? (t as AilpAssessmentMode)
    : undefined;
}

function coerceSecurityFlag(raw: string | undefined): boolean | undefined {
  if (raw == null) return undefined;
  const t = raw.trim().toLowerCase();
  if (t === "") return undefined;
  if (["1", "true", "yes", "on", "security"].includes(t)) return true;
...
interface EnvBag {
  ...
  assessmentMode: string | undefined;
...
export function resolveAilpConfigFromEnv(overrides?: Partial<AilpOptions>): AilpOptions {
  ...
  if (apiKey === "") {
    throw new Error(
      "AILP: missing `apiKey`. Get a key at https://ailp.airtasystems.com and set NEXT_PUBLIC_AILP_API_KEY (Next.js) or VITE_AILP_API_KEY (Vite), or pass `apiKey` to useAilp(). Restart the dev server after changing env so it is inlined. Security: browser-exposed keys are public — prefer calling AILP from a server route.",
    );
  }
  ...
  if (programId === "") {
    throw new Error(
      "AILP: missing `programId`. Copy your program ID from ailp.airtasystems.com and set NEXT_PUBLIC_AIRTASYSTEMS_PROGRAM_ID (Next.js) or VITE_AIRTASYSTEMS_PROGRAM_ID (Vite), or pass `programId` to useAilp().",
    );
  }
